...
class MainApp(QWidget):
    def __init__(self, user):
        ...
        self.transactions_list = QListWidget()
        self.layout.addWidget(QLabel('История транзакций:'))
        self.layout.addWidget(self.transactions_list)

        if user.role == 'admin':
            self.admin_button = QPushButton('Интерфейс админа', self)
            self.admin_button.clicked.connect(self.show_admin_interface)
